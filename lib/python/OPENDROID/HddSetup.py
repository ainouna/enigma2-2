# Embedded file name: /usr/lib/enigma2/python/OPENDROID/HddSetup.py
...
class HddSetup(Screen):
    ...
    def yellowAswer(self):
        if sfdisk and len(self.mdisks.disks) > 0:
            ...
            self.session.openWithCallback(self.chooseFSType, ExtraMessageBox, _('Please select your preferred configuration.') + '\n' + _("Or use standard 'Harddisk Setup' to initialize your drive in ext4."), _('Partitioner'), [[_('One partition'), 'partitionmanager.png'],
...
class HddPartitions(Screen):
    ...
    def yellow(self):
        if sfdisk and len(self.disk[5]) > 0:
            ...
            if self.disk[5][self.index][3] == '83':
                if self.isExt4Supported():
                    self.session.openWithCallback(self.domkfs, ExtraMessageBox, _('Format as'), _('Partitioner'), [['Ext4', 'partitionmanager.png'],
                     ['Ext3', 'partitionmanager.png'],
                     ['Ext2', 'partitionmanager.png'],
                     [_('Cancel'), 'cancel.png']], 1, 3)
                else:
                    self.session.openWithCallback(self.domkfs, ExtraMessageBox, _('Format as'), _('Partitioner'), [['Ext3', 'partitionmanager.png'], ['Ext2', 'partitionmanager.png'], [_('Cancel'), 'cancel.png']], 1, 2)
            elif self.disk[5][self.index][3] == '7':
                self.session.openWithCallback(self.domkfs, ExtraMessageBox, _('Format as'), _('Partitioner'), [['NTFS', 'partitionmanager.png'], ['exFAT', 'partitionmanager.png'], [_('Cancel'), 'cancel.png']], 1, 2)
            elif self.disk[5][self.index][3] == 'b' or self.disk[5][self.index][3] == 'c':
...
class HddInfo(ConfigListScreen, Screen):
    ...
    def __init__(self, session, device, deviceinfo):
        ...
        self['actions'] = ActionMap(['OkCancelActions', 'ColorActions'], {'red': self.keyCancel,
         'green': self.keySave,
