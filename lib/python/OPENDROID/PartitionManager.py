# Embedded file name: /usr/lib/enigma2/python/OPENDROID/PartitionManager.py
    ...
    try:
        ...
        for x in result.split('\n'):
            if x.find('BYT;') >= 0:
                ...
                addok = False
                entry = []
            elif addok and len(x) > 1 and x[len(x) - 1] == ';':
                l = x.split(':')
                if len(l) == 7:
...
class Cpart(Screen):
    ...
    def __addFreePart(self, plist, lastPartEnd):
        x = [LIST_TYPE_PAR,
         ...
         0,
         _('free'),
         self.PA_TYPE_FREE | self.PA_TYPE_LAST,
         ';']
        plist.append(x)
    ...
    def __addPart2Comlist(self, list, val, mkpart = True):
        ...
        if mkpart:
            fs = val[PA_FS]
            com = 'parted -s -a optimal %s mkpart primary %s %s%s %s%s | sleep 35 | echo -e  y' % (self.__devpath,
             fs,
             val[PA_START],
             self.__unit,
             val[PA_END],
        ...
        list.append((com, _("make filesystem '%s' on partition %s (%d %s)") % (val[PA_FS],
          partnr,
          val[PA_SIZE],
          self.__unit), mountdev))
        return
            ...
            if self.__new_part_list[x][LIST_TYPE] == LIST_TYPE_PAR:
                if bool(self.__new_part_list[x][PA_TYPE] & self.PA_TYPE_FREE) == False:
                    if len(self.__old_part_list) > x and bool(self.__old_part_list[x][PA_TYPE] & self.PA_TYPE_FREE) == False:
                        ...
                        elif self.__new_part_list[x][PA_FS] != self.__old_part_list[x][PA_FS]:
...
class Cpartexe(Screen):
    ...
    def __exeList(self):
        if len(self['list'].list) > self.__state and self.__state > -1:
            ...
            if res == 0:
                pic = 'test_true.png'
            self['list'].list[self.__state] = (self['list'].list[self.__state][0],
             LoadPixmap(path=SkinDefaultPath + pic),
             self['list'].list[self.__state][2],
             self['list'].list[self.__state][2])
            ...
            if res == 0:
                ...
                self.__state = len(self['list'].list)
