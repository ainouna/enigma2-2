# Embedded file name: /usr/lib/enigma2/python/OPENDROID/AddonsPanel.py
...
class AddonsRemove(Screen):
    ...
    def __init__(self, session, type = 1, needupdate = True):
        ...
        self.run = 0
        self.remainingdata = ''
        self['actions'] = ActionMap(['WizardActions', 'ColorActions'], {'ok': self.go,
        ...
        if val:
            if self.type == self.DOWNLOAD:
                if pluginnames.startswith('enigma2-plugin-picons-'):
                    ...
                    return
                if pluginnames.startswith('enigma2-plugin-display-picon'):
                    supported_filesystems = frozenset(('vfat', 'ext4', 'ext3', 'ext2', 'reiser', 'reiser4', 'jffs2', 'ubifs', 'rootfs'))
                    candidates = []
                    import Components.Harddisk
                    mounts = Components.Harddisk.getProcMounts()
...
class downloadTaskPostcondition(Condition):
    ...
    def getErrorMessage(self, task):
        return {task.ERROR_CORRUPT_FILE: _('Video Download Failed!Corrupted Download File:%s' % task.lasterrormsg),
         ...
         task.ERROR_UNKNOWN: _('Download Failed!Unknown Error:%s' % task.lasterrormsg)}[task.error]